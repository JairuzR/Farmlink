...
<section class="bg-slate-50 min-h-screen py-10">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-4 flex gap-2 border-b border-slate-200">
            <a href="{{ route('cart') }}" class="flex items-center gap-1.5 border-b-2 border-transparent px-4 py-2.5 text-sm font-medium text-slate-500 hover:text-slate-700">
                🛒 Cart
            ...
            <a href="{{ route('orders.pending') }}" class="flex items-center gap-1.5 border-b-2 border-green-600 px-4 py-2.5 text-sm font-medium text-green-700">
                Pending
                @if($orders->isNotEmpty())
                    <span class="rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700">{{ $orders->count() }}</span>
                @endif
            </a>
            <a href="{{ route('orders.delivered') }}" class="flex items-center gap-1.5 border-b-2 border-transparent px-4 py-2.5 text-sm font-medium text-slate-500 hover:text-slate-700">
                Delivered
            </a>
        </div>
        ...
        <div class="mt-6 space-y-4">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50 px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="rounded-full px-3 py-1 text-xs font-semibold
                                ...
                                @elseif(in_array($order->status, ['confirmed','preparing'])) bg-blue-100 text-blue-700
                    ...
                    <div class="divide-y divide-slate-100 px-6">
                        @foreach($order->items as $item)
                            ...
                            <div class="flex items-center gap-4 py-4">
                                @php
                                    $thumb = $product && $product->primaryImage
            ...
            @else
                <div class="rounded-xl bg-white py-16 text-center shadow-sm ring-1 ring-slate-200">
                    ...
                    <a href="{{ route('marketplace') }}" class="mt-5 inline-block rounded-lg bg-green-600 px-6 py-3 font-semibold text-white hover:bg-green-700">
                        Browse Marketplace
                    </a>
                </div>
            @endif
